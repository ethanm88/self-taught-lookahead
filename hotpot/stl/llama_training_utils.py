...
'''
    Use the faster LLaMA unsloth library (rewrites gradient computation with OpenAI Triton)
'''
...
def train_llama_model(model, tokenizer, train_dataset, eval_dataset, checkpoint_dir, logging_dir, save_dir, max_seq_length=8192, loss_for_save="loss", num_epochs=10, learning_rate=2e-4, depth=1):
    # Set up the trainer
    trainer = SFTTrainer(
        model = model,
...
def fetch_raw_training_file_name(idx, training_data_dir, start_idx=0, num_samples=50, ratio=0.75, model_name="unsloth/Meta-Llama-3.1-8B-Instruct", backend="gpt-3.5-turbo"):
...
def fetch_formatted_training_file_name(idx, training_data_dir, start_idx=0, num_samples=50, ratio=0.75, model_name="unsloth/Meta-Llama-3.1-8B-Instruct", depth=None, backend="gpt-3.5-turbo"):
...
def fetch_formatted_eval_file_name(idx, training_data_dir, start_idx=0, num_samples=50, ratio=0.75, model_name="unsloth/Meta-Llama-3.1-8B-Instruct", depth=None, backend="gpt-3.5-turbo"):
    ratio_str = get_ratio_str(ratio)
    formatted_model_name = get_formatted_model_name(model_name)
    backend_contribution = get_backend_contribution(backend)
