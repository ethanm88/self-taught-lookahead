...
parser.add_argument('--metric', choices=['match_rate'], default='match_rate')
...
def load_rewards(file, num):
    with open(file, "r") as f:
        ...
        return [(int)(entry["success"]) for entry in data if "success" in entry][:num]
...
def compute_metric(results, metric, num):
    if metric == "match_rate":
        ...
        raise ValueError("Invalid metric. match_rate is the only allowable metric")
